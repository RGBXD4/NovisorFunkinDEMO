...
class FreeplayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		if (FlxG.save.data.fcList == null)
			FlxG.save.data.fcList = [];
		if (FlxG.save.data.fcDiff == null)
			FlxG.save.data.fcDiff = [];
	...
	override function update(elapsed:Float)
	{
		...
		if (trans != null)
			CustomFadeTransition.nextCamera = trans; // putting this as create() won't work
			...
			if (isDisked)
			{
				if (!stopSpamming)
				{
					...
					else
					{
						...
						FlxTween.tween(this, {diskSpd: CoolUtil.getFPSUpdateShitOrWhatever(7.5)}, 1, {
							ease: FlxEase.linear,
							onComplete: function(fuck:FlxTween)
							{
								FlxTween.tween(infoTxt, {y: FlxG.height}, time, {ease: ease});
								FlxTween.tween(disk, {x: disket.x}, time, {
									ease: FlxEase.backIn,
									onComplete: function(t:FlxTween)
									{
										diskSpd = 0;
										FlxTween.tween(disket, {x: disket.x + 10}, 0.125, {ease: ease});
										FlxTween.tween(disk, {x: disk.x + 10}, 0.125, {
											ease: ease,
											onComplete: function(a:FlxTween)
											{
												FlxTween.tween(disket, {x: disket.x - 10}, 0.25, {ease: FlxEase.quadOut});
												FlxTween.tween(disk, {x: disk.x - 10}, 0.25, {
