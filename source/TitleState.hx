...
class TitleState extends MusicBeatState
{
	...
	override public function create():Void
	{
		...
		Application.current.window.title = "Novisor Funkin' - Title Screen";

		var shader:Shaders.ChromaticAberrationEffect = new Shaders.ChromaticAberrationEffect();
		shader.setChrome(0.002);
	...
	function startIntro()
	{
		...
		starFG = new FlxSprite().loadGraphic(Paths.image('mainmenu/mainmenuV4/starFG'));
		starFG.updateHitbox();
		starFG.antialiasing = true;
		starFG.scrollFactor.set();
		add(starFG);
		FlxTween.tween(starFG, {x: -2000}, 400);

        starBG = new FlxSprite().loadGraphic(Paths.image('mainmenu/mainmenuV4/starBG'));
		starBG.updateHitbox();
		starBG.antialiasing = true;
		starBG.scrollFactor.set();
		add(starBG);
		FlxTween.tween(starBG, {x: -1000}, 600);

        titlelogo = new FlxSprite(0, 45.75);
        titlelogo.frames = Paths.getSparrowAtlas('title_screen/logo_bump');
        titlelogo.animation.addByPrefix('bump', 'logobump', 24, false);
        titlelogo.animation.play('bump');
		titlelogo.antialiasing = true;
        titlelogo.screenCenter(X);
        titlelogo.updateHitbox();
        add(titlelogo);

        enter = new FlxSprite(120, 603.65);
        enter.frames = Paths.getSparrowAtlas('title_screen/enter');
        enter.animation.addByPrefix('idle', 'pressenter', 24, true);
		enter.animation.addByPrefix('selected', 'SELECTED', 24, false);
        enter.animation.play('idle');
        enter.updateHitbox();
        add(enter);
        
        lights = new FlxSprite(-1126.3, -896.75).loadGraphic(Paths.image('title_screen/lights'));
        lights.updateHitbox();
        //add(lights);
	...
	override function update(elapsed:Float)
	{
		...
		if (initialized && !transitioning && skippedIntro)
		{
			...
			if(pressedEnter)
			{
				...
				if(enter != null) enter.animation.play('selected', false);
				enter.offset.set(50, 75);
	
				titlelogo.animation.play('bump', false);
	...
	override function beatHit()
	{
		...
		FlxTween.tween(FlxG.camera, {zoom: 1.1}, 0.5, {ease: FlxEase.expoOut, type: BACKWARD});
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				...
				case 6:
					#if PSYCH_WATERMARKS
					createCoolText(['The is associated', 'with'], -40);
					#else
					createCoolText(['In association', 'with'], -40);
					#end
				case 8:
					addMoreText('gametoons', -40);
					ngSpr.visible = true;
				// credTextShit.text += '\nNewgrounds';
				case 9:
					deleteCoolText();
